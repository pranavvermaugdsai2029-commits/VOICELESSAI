...
class DETRData(Dataset): 
    ...
    def safe_transform(self, image, bboxes, class_labels, max_attempts=50):
     for attempt in range(max_attempts):
        try:
            ...
            # ensure at least one bbox survives
            if len(transformed['bboxes']) > 0:
                return transformed
        except Exception:
            continue

     # Fallback: force deterministic transform (guaranteed tensor + size)
    ...
    def __getitem__(self, idx): 
        ...
        if os.path.exists(jpg_path):
         ...
         raise FileNotFoundError(f"Image not found for {self.image_name}")
        ...
        augmented = self.safe_transform(image=np.array(img), bboxes=bounding_boxes,class_labels=class_labels)
